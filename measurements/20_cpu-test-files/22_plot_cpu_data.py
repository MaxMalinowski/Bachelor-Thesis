...
def get_file(choice=None):
    ...
    if choice == "1":
        file_idle = master_path  + "/rcar_compute_base/cpu-test-v1.json"
        file_load = master_path  + "/rcar_compute_load/cpu-test-v1.json"
        name = "cpu_test_rcar_compute.pdf"
    elif choice == "2":
        file_idle = master_path  + "/xeon_compute_base/cpu-test-v1.json"
        file_load = master_path  + "/xeon_compute_load/cpu-test-v1.json"
        name = "cpu_test_xeon_compute.pdf"
    elif choice == "3":
        file_idle = master_path  + "/xeon_controller_base/cpu-test-v1.json"
        file_load = master_path  + "/xeon_controller_load/cpu-test-v1.json"
        name = "cpu_test_xeon_controller.pdf"
    else:
...
def main():
    ...
    for i in range(3):
        ...
        ax[i].set_xticklabels(['Linux Kernel Compilation'])
    ...
    plt.show()
    fig.savefig(os.path.dirname(os.path.realpath(__file__)) + '/../100_results/102_cpu_test/all-cpu.pdf', transparent=True)
